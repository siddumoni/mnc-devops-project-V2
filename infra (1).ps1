...
function Import-JenkinsEBS {
    ...
    Push-Location $ENV_DIR
    try {
        # FIX: Check if already in state to make this call idempotent.
        # terraform import errors if you try to import a resource already tracked.
        ...
        # FIX: Check $LASTEXITCODE — terraform import does not throw a
        # PowerShell exception on failure, it only sets the exit code.
        # The original try/catch never actually caught terraform failures.
        if ($LASTEXITCODE -ne 0) {
            throw "terraform import failed for EBS volume $ebsVolumeId. Cannot continue — a new EBS would be created, losing your Jenkins data. Check state manually."
        }
